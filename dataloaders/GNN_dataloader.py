...
from global_parameters import LANGUAGE_EMBEDDING_CACHE_DIR, MAX_SEQ_LEN
...
class GNNDataloaderInitializer():
    def __init__(self, reset_g_epoch=False, num_workers=20, use_sampler=False, augmentation_method=None,
                 device="cpu", use_linegraph=False, use_raw_text_attributes=False,
                 existing_dataloader_intializing_instance=None):
        ...
        self.use_raw_text_attributes = use_raw_text_attributes


    def build_dataloader(self, dataset, train_val_test=None, existing_dataloader_intializing_instance=None):
        self.existing_dataloader_intializing_instance = existing_dataloader_intializing_instance
        ...
        if self.use_raw_text_attributes:
            efeats = dataset.efeats
            efeats = efeats.tolist()
            CM.Utilities.TimeElapsed()
            if self.use_raw_text_attributes:
                if self.existing_dataloader_intializing_instance is None:
                    self.tokenizer = DistilBertTokenizerFast.from_pretrained("distilbert-base-uncased",
                                                                             do_lower_case=True,
                                                                             cache_dir=LANGUAGE_EMBEDDING_CACHE_DIR)
                    self.text_encoded_dict = self.tokenizer.batch_encode_plus(
                            efeats,  # Sentence to encode.
                    ...
                    self.text_encoded_dict = self.existing_dataloader_intializing_instance.text_encoded_dict
            dataset.efeats = self.text_encoded_dict
...
if __name__ == "__main__":
    dataset = DBLPEdgeAttributed(subset="train", add_self_loop=False, randomize_train_test=False,
                                 ...
                                 reduced_dim=256, is_bipartite=True)
    train_dataloader = GNNDataloaderInitializer(use_raw_text_attributes=True).build_dataloader(dataset)
