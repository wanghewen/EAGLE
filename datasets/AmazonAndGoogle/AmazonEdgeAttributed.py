...
class AmazonEdgeAttributed(EdgeAttributed):
    ...
    def __init__(self, subset=None, no_node_features=False, initialized_instance=None, add_self_loop=True,
        ...
        if is_bipartite:
            data_folder = os.path.join(AmazonBipartiteTextDataFolder, product_category)
        else:
            data_folder = AmazonReviewCountVectorizerDataFolder
        super().__init__(name="AmazonEdgeAttributed", data_folder=data_folder,
...
if __name__ == "__main__":
    dataset = AmazonEdgeAttributed(subset="train", is_bipartite=True, process_text=True, use_text_cache=True,
